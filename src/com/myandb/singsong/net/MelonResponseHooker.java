...
public class MelonResponseHooker {
	...
	public static void hook(Context context, JSONObject response) throws MelonResponseException, JSONException {
		hook(context, null, response);
	}
	
	public static void hook(Context context, FragmentManager fm, JSONObject response) throws MelonResponseException, JSONException {
				...
				if (fm != null) {
					...
					if (errorCode.equals("ERL003")) {
						...
						if (isJuvenileProtection.equals("Y")) {
							...
							try {
								url += "&returnUrl=" + URLEncoder.encode("singsong://root", "UTF-8");
