...
jint Java_com_myandb_singsong_libogg_VorbisFileInputStream_create(
		JNIEnv* env,
		jobject this,
		jstring path,
		jobject info
...
jint Java_com_myandb_singsong_libogg_VorbisFileInputStream_readStreamIdx(
		JNIEnv* 	env,
		jobject 	this,
		jint		sidx,
		jshortArray pcm,
		...
		)
{
	...
	if (length > 0) {
		ret = ov_read(&iptr->vf, (char *)(pcmShorts + offset), length, &iptr->section);
		/* -1 is EOF */
		if (ret == 0) {
			ret = -1;
		}
...
jlong Java_com_myandb_singsong_libogg_VorbisFileInputStream_skipStreamIdx(
		JNIEnv* 	env,
		jobject 	this,
		jint		sidx,
		jlong 		offset
		)
{
	...
	ret = ov_pcm_seek(&iptr->vf, offset);
...
void Java_com_myandb_singsong_libogg_VorbisFileInputStream_closeStreamIdx(
		JNIEnv* 	env,
		jobject 	this,
		jint		sidx
		)
